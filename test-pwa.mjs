...
assert(manifest.display_override?.[0] === "fullscreen", "installed app prefers fullscreen");
assert(manifest.display_override.includes("standalone"), "fullscreen falls back to standalone");
...
assert(app.includes('wakeLock.request("screen")'), "screen Wake Lock keeps the display on");
assert(app.includes('navigationUI: "hide"'), "installed phone app hides system navigation");
assert(app.includes("pointer: coarse"), "fullscreen navigation hide stays on phones");
assert(html.includes('name="apple-mobile-web-app-capable"'), "iOS home screen hides Safari navigation");
assert(html.includes('name="mobile-web-app-capable"'), "installed app is allowed to run without browser chrome");
assert(app.includes("function holdScreenAwake"), "wake lock request is centralized");
assert(app.includes('addEventListener("visibilitychange"'), "wake lock is re-acquired when the app is shown again");
...
assert(app.includes("./#home/stay"), "home trap uses a distinct hash Chrome will not skip");
assert(app.includes('return name === "setup" ? "./#home"'), "home is not the launch ./ URL");
assert(app.includes("swallowHomeNavigate"), "Navigation API intercepts home Back");
assert(app.includes('addEventListener("navigate", swallowHomeNavigate'), "home Back intercept is registered");
const popHandler = app.slice(app.indexOf("function handleHistoryPop"), app.indexOf("function swallowHomeNavigate"));
...
assert(app.includes("round.streak = 0"), "a miss breaks the streak");
const leaveOpen = app.slice(app.indexOf("function setLeaveOpen"), app.indexOf("function isLeaveOpen"));
assert(leaveOpen.includes("history.back()"), "closing the leave dialog pops its history entry");
assert(!leaveOpen.includes("replaceState"), "staying does not keep a duplicate play entry");
assert(html.includes("raw?.settings?.theme"), "legacy light theme applies before the first paint");
assert(!html.includes('id="keypad" aria-hidden="true"'), "the keypad is available to assistive tech");
