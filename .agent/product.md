...
2. Difficulty: Pictures, Easy, Medium, Hard. Pictures shows kid-friendly drawings of fruit, toys, and school supplies to count. Addition and subtraction stay at 8 or below. Multiplication uses factors 2–9. Pictures has no division.
...
Answers are entered only with the on-screen keypad. Digit rows are **1–3**, then **4–6**, then **7–9**. The **Next** / **Submit** button sits below the keypad. Multi-step multiplication and division use **Next** to move to the next working section; the last section uses **Submit**. Multiplication is filled one digit at a time from right to left, including carries. Each partial row is the full product of the top number times that bottom digit (`324 × 7` is `2268`; `26 × 2` is `52`: write `2`, carry `1`, tens `5`). Add each carry into the next place so `24 × 8` is `192` and `9 × 3` is `27`. Every carry field — multiplication, addition, and subtraction — sits in a small circle, two type sizes smaller than the working digits, in a cursive face. The first step’s carries go immediately above the top number; each later step stacks a new carry row above that. The final sum also has circled carry boxes when a column adds to 10 or more. A tens partial is that full product shifted left — fill `9` for `9 × 1`, not `90`. Do not write placeholder zeros on the right. Long division fills one quotient digit at a time, then that step’s multiply, subtract, and bring-down boxes. Medium and hard addition and subtraction fill one digit at a time from right to left, and a carry or borrow box whenever that column needs one. Pictures and easy keep one blank. There is no separate final-only answer box, and the keypad must not open a system keyboard.
...
Easy is a flat equation (`15 ÷ 3 = _`, `5 × 3 = __`), not a column stack. Medium and hard addition and subtraction are vertical column arithmetic, filled from right to left, with a carry or borrow box when a column needs one. Multiplication is written as long multiplication with the larger factor on top and no extra `a × b =` line, except easy multiplication, which stays the flat `[2–9] × [2–9]` equation. Division is written as long division, with bring-down working and no times-table cheat, except easy division, which is a flat `[10–99] ÷ [2–9]` equation and skips `X ÷ X` plus dividends made only of the divisor digit. Medium division is a 3–5 digit dividend by a one-digit divisor 3–9. Hard division is a 4–7 digit dividend by a two-digit divisor. Medium multiplication is a 2–3 digit factor by a one-digit 3–9. Hard multiplication is a 3–5 digit factor by a 2–3 digit factor. Picture multiplication uses factors 2–9. Addition, subtraction, multiplication, and division never use 0 or 1 as an operand (`+ 1`, `− 0`, `× 1`, `÷ 1` are not generated). Subtraction is never `X − X`. Worksheet misses do not rewrite the problem as `220 ÷ 20 = 11` in the feedback line.
