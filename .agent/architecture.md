...
6. The keypad writes `current.fills[current.active]`. Multiplication slots are one digit (or carry) and advance right-to-left. Division slots are one digit each for quotient, multiply, subtract, and bring-down. Submit uses `fieldsReady` / `fieldsMatch` so every slot must be filled and correct. Practice retries on a miss; quiz and sprint advance after one try.
