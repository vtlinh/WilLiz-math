...
2. Each person (Will, Liz, Guest) has their own mix: `ops`, `difficulty`, `mode`, `theme`. Theme defaults to `dark`. Switching learners restores that mix immediately. The avatar and settings button are hidden during a round. The settings page saves each tap inline; ← in the action bar returns to the home page. During play, ← always asks to confirm before showing results. Screen changes use `history.pushState`. On home, extra `#home-*` history entries plus a Navigation API traverse intercept swallow system back so the app does not quit. Chrome skips script-only history entries, so intercepting `navigate` is what keeps a same-origin Back on the home page.
